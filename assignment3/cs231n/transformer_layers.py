...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query(query).reshape(N, S, H, Hd).transpose(1, 2) 
        key = self.key(key).reshape(N, T, H, Hd).transpose(1, 2)        
        value = self.value(value).reshape(N, T, H, Hd).transpose(1, 2)  
        
        attn_map = torch.matmul(query, key.transpose(2, 3)) / Hd**0.5 

        if attn_mask is not None:
            attn_map = attn_map.masked_fill(attn_mask == 0, -torch.inf)

        attn_map = F.softmax(attn_map, dim=3)
        attn_map = self.attn_drop(attn_map)
        
        y = torch.matmul(attn_map, value).transpose(1, 2).reshape(N, S, E) 
        output = self.proj(y)
